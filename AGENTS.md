...
узле мешают, нужен `kubernetes_dir` на заведомо пустой каталог, а не надежда, что рядом
ничего не лежит.
...
подпись. Инварианты закреплены в `tests/test_mermaid_validity.py`,
`tests/test_node_grouping.py` и `tests/test_service_node.py`.
...
- Новая поддержанная технология доезжает до `tests/showcase`: `tests/test_playground.py`
  требует, чтобы каждая возможность была видна на его странице. Оттуда же снят
