...
│       ├── discovery.py # Поиск чарта на диске (от root_dir, не от cwd) и чтение манифестов
...
Поиск чарта всегда стартует от `root_dir`, который задал пользователь: сначала `Chart.yaml`
внутри самого `root_dir` (до трёх уровней вложенности), затем `deploy/`, `helm/`, `charts/`,
`.helm/` на один-три уровня выше. Подъём останавливается на каталоге с маркером проекта
(`.git`, `pyproject.toml`, `setup.py`, `setup.cfg`), чтобы не утащить чарт соседнего проекта
или что-то из корня файловой системы. Явный `helm_chart_dir` с относительным путём тоже
резолвится от `root_dir`, а не от cwd процесса: cwd у запущенного приложения может быть любым.
