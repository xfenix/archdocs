...
async def write_rate_limit(
    ...
    if db_rate_limit:
        msg = "Rate Limit Name not available"
...
async def patch_rate_limit(
    ...
    if db_rate_limit_path:
        msg = "There is already a rate limit for this path"
        raise DuplicateValueException(msg)

    await crud_rate_limits.exists(db=db)
    if db_rate_limit_path:
        msg = "There is already a rate limit with this name"
        raise DuplicateValueException(msg)
